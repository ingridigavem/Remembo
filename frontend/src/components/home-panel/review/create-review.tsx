...
import {
  createContent,
  fetchDashboard,
} from "@/redux/features/dashboard/thunk";
import { selectSubjectOptions } from "@/redux/features/subjects/subjectSlice";
...
export function CreateReview() {
  ...
  const subjects = useAppSelector(selectSubjectOptions);
  ...
  async function handleAddSubject(name: string) {
    await dispatch(createSubject({ name }));
  }
  ...
  useEffect(() => {
    const newSubject = subjects.filter((m) => m.label === search);
    if (newSubject.length > 0) {
      form.setValue("subjectId", newSubject[0].value);
      setOpen(false);
    }
  }, [form, subjects]);
                    ...
                    <Popover open={open} onOpenChange={setOpen}>
                      ...
                      <PopoverContent className="w-[380px] p-0">
                        <Command>
                          ...
                          <CommandList>
                            <CommandEmpty>
                              <Button onClick={() => handleAddSubject(search)}>
